...
async def on_ready(): 
    ...
    await client.change_presence(
                                         ...
                                         activity=discord.Game(name="자라가 자라하는 중"))  
...
@client.command() 
async def 그리드(ctx, *args):
    await ctx.send("이 서버의 부관리자야! 부관리자는 임시라서 나중에 해고될 가능성이...(속닥속닥)")

@client.command() 
async def Greed(ctx, *args):
    await ctx.send("이 서버의 부관리자야! 부관리자는 임시라서 나중에 해고될 가능성이...(속닥속닥)")

@client.command() 
async def Eva(ctx, *args):
    await ctx.send("이 서버의 부관리자야!")
...
async def 관리자(ctx, *args):
    await ctx.send("우리 서버의 총관리자는 Steel님, 부관리자는 리니님, Greed님, Eva님이야!")
